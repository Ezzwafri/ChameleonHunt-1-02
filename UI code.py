...
class GameUI:
    ...
    def create_powerup_buttons(self):
        ...
        if self.game_logic.add_time_uses > 0:
            self.add_time_btn = tk.Button(self.powerup_frame, text=f"Add Time ({self.game_logic.add_time_uses})", command=self.game_logic.use_add_time, bg="#32CD32", fg="black", font=("Arial", 12, "bold"), relief="raised")
        ...
        if self.game_logic.add_steps_uses > 0:
            self.add_steps_btn = tk.Button(self.powerup_frame, text=f"Add Steps ({self.game_logic.add_steps_uses})", command=self.game_logic.use_add_steps, bg="#FF69B4", fg="black", font=("Arial", 12, "bold"), relief="raised")
    ...
    def toggle_pause(self):
        ...
        if self.paused:
            ...
            self.stop_timer()
            if hasattr(self, 'add_time_btn'):
                self.add_time_btn.config(state="disabled")
            if hasattr(self, 'add_steps_btn'):
                self.add_steps_btn.config(state="disabled")
            self.feedback.config(text="Game Paused", fg="#FFA500")
        else:
    ...
    def update_powerup_buttons(self):
        if hasattr(self, 'add_time_btn'):
            self.add_time_btn.config(text=f"Add Time ({self.game_logic.add_time_uses})", state="disabled" if self.paused or self.game_logic.add_time_uses == 0 else "normal", bg="#A9A9A9" if self.paused or self.game_logic.add_time_uses == 0 else "#32CD32")
        if hasattr(self, 'add_steps_btn'):
            self.add_steps_btn.config(text=f"Add Steps ({self.game_logic.add_steps_uses})", state="disabled" if self.paused or self.game_logic.add_steps_uses == 0 else "normal", bg="#A9A9A9" if self.paused or self.game_logic.add_steps_uses == 0 else "#FF69B4")
