...
class GameLogic:
    ...
    def use_add_time(self):
        if self.ui.points < 20 or self.found or not self.ui.timer_running:
            return
        self.ui.points -= 20
        difficulty = self.ui.difficulty.get()
    ...
    def use_add_steps(self):
        if self.ui.points < 15 or self.found:
            return
        self.ui.points -= 15
...
class GameUI:
    ...
    def create_powerup_buttons(self):
        ...
        self.add_time_btn = tk.Button(self.powerup_frame, text="Add Time (-20)", command=self.game_logic.use_add_time, 
        ...
        self.add_steps_btn = tk.Button(self.powerup_frame, text="Add Steps (-15)", command=self.game_logic.use_add_steps, 
    ...
    def update_powerup_buttons(self):
        time_state = "normal" if self.points >= 20 and not self.paused and not self.game_logic.found and self.timer_running else "disabled"
        steps_state = "normal" if self.points >= 15 and not self.paused and not self.game_logic.found else "disabled"
        
        self.add_time_btn.config(state=time_state, bg="#32CD32" if time_state == "normal" else "#A9A9A9")
        self.add_steps_btn.config(state=steps_state, bg="#FF69B4" if steps_state == "normal" else "#A9A9A9")
